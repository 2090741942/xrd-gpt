...
class PrintGPUUsageCallback(TrainerCallback):
    ...
    def __init__(self, *args, interval_step=1, **kwargs):
        super().__init__(*args, **kwargs)
        self.interval_step = interval_step

    def on_step_end(self, args, state, control, **kwargs):
        if state.global_step % self.interval_step == 0:
            try:
...
class ExampleTrainerCallback(TrainerCallback):
    ...
    def on_step_end(
        ...
        **kwargs,
        ...
        if self.run_every == "step" and state.global_step % self.step_interval == 0:
