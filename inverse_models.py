...
def main(config_file=None):
    ...
    with open(id_prop_path_train, "r") as f:
        reader = csv.reader(f)
        dt_train = [row for row in reader]

    with open(id_prop_path_validation, "r") as f:
        reader = csv.reader(f)
        dt_val = [row for row in reader]
    ...
    dat_train = []
    ids_train = []
    for i in tqdm(dt_train, total=len(dt_train)):
        info = {}
        info["id"] = i[0]
        ids_train.append(i[0])
        ...
        pth = os.path.join(run_path_train, info["id"])
        ...
        dat_train.append(info)

    # val_data loader
    dat_val = []
    ids_val = []
    for i in tqdm(dt_val, total=len(dt_val)):
        info = {}
        info["id"] = i[0]
        ids_val.append(i[0])
        tmp = [float(j) for j in i[1:]]
        ...
        pth = os.path.join(run_path_val, info["id"])
        if config.file_format == "poscar":
            atoms = Atoms.from_poscar(pth)
        elif config.file_format == "xyz":
            atoms = Atoms.from_xyz(pth)
    ...
    train_ids = ids_train
    print("num_train", len(dat_train))
    print("num_test", len(dat_val))
    test_ids = ids_val
    ...
    #     "json",
    #     data_files={
    #         "train": alpaca_prop_train_filename,
    #         "validation": alpaca_prop_test_filename
    #     }
    # )
